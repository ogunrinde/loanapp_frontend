...
import {  GetBorrowerOverdues } from '../redux/action/index';
import { Tab, Tabs, TabList, TabPanel } from 'react-tabs';
import 'react-tabs/style/react-tabs.css';
import Request from './Request';

const BorrowerOverDues = (props) => 
{
	const dispatch = useDispatch();
    //const IsFetching = useSelector(state => state.root.IsFetching);
    const borrower_overdues = useSelector(state => state.root.borrower_overdues);
    ...
    {
        setIsFetching(true);
        await dispatch(GetBorrowerOverdues());
        setIsFetching(false);
    }
    ...
    return(
        <section className="lattest-product-area pb-40 category-list">
            <ReactNotification />
                ...
                { 
                    borrower_overdues != null && borrower_overdues.length > 0 &&        
                        <tbody>
                        {
                            borrower_overdues.map((overdue,id) =>
                            <tr>
                                <td>
                                    <p>{id+1}</p>
                                </td>
                ...
                {
                    borrower_overdues != null && IsFetching == false && borrower_overdues.length == 0 && 
...
export default BorrowerOverDues;