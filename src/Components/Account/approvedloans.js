...
import {  GetApprovedLoans } from '../redux/action/loan';
import { Tab, Tabs, TabList, TabPanel } from 'react-tabs';
import 'react-tabs/style/react-tabs.css';
import ApprovedLoanRequest from './ApprovedLoanRequest';

const ApprovedLoans = (props) => 
{
	const dispatch = useDispatch();
    const IsFetching = useSelector(state => state.root.IsFetching);
    const LenderApprovedLoans = useSelector(state => state.root.LenderApprovedLoans);
    const [IsFetchingApprovedloans, setIsFetchingApprovedloans] = useState(false);
    ...
    const process = async  () =>
    {
        setIsFetchingApprovedloans(true);
        await dispatch(GetApprovedLoans());
        setIsFetchingApprovedloans(false);
    }
        ...
        <section className="lattest-product-area pb-40 category-list">
            <ReactNotification />
                <div hidden={!view_more} className="sideview">
                    ...
                    <div className="">
                        <div id="">
                        { Object.keys(request).length > 0 && <ApprovedLoanRequest request = {request} type = {type} />   }
				...
				<div hidden = {IsFetchingApprovedloans} className="table-responsive ">
                    <table className="table">
                        ...
                        <tbody>
                        {
                            LenderApprovedLoans.map((request,id) =>
                            <tr>
                                <td>
                                    {id+1}
                                </td>
